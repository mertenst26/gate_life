...
export function ScenarioBuilder({ onBack, editScenarioId }: Props) {
	...
	const [dungeonContext, setDungeonContext] = useState<{
	...
	const handleEditEntity = useCallback(() => {
		...
		if (selectedTemplate.entity_type === "dungeon") return; // dungeons use DungeonDesignerPanel
		console.log(
			`[ScenarioBuilder] Opening edit chat for "${selectedTemplate.name}" (${selectedTemplate.id})`,
		);
		setChatContext({
			entityType: selectedTemplate.entity_type as "enemy" | "npc" | "friendly" | "vehicle" | "poi",
	...
	const handleEntityConfirmBatch = useCallback(
		...
		// Reuse the same cluster logic as single-type multi-placement
		const positions = clusterPositions(
			chatContext.lat,
			chatContext.lng,
			entities.length,
	...
	return (
		<div className="scenario-builder fade-in">
			...
			{step === "map" && scenarioId && (
				<div className="scenario-map-step fade-in">
					...
					<div className="scenario-info-field">
						...
						<div style={{ display: "flex", alignItems: "center", gap: "0.4rem", marginTop: "0.4rem" }}>
							<label className="text-xs text-dim" style={{ whiteSpace: "nowrap" }}>
								Wandering Monster
							</label>
							<input
								type="checkbox"
							...
							{wanderingMonsterConfig?.enabled && (
								<>
									<input
										...
										onChange={(e) => {
											const v = Math.min(100, Math.max(0.1, Number(e.target.value)));
											setWanderingMonsterConfig((prev) => prev ? { ...prev, encounter_chance: Math.round(v * 10) / 10 } : prev);
										}}
										onBlur={() => {
											if (scenarioId && wanderingMonsterConfig) {
												api
													.saveWanderingMonsterConfig(scenarioId, wanderingMonsterConfig)
													.catch(console.error);
											}
										}}
									...
									{wanderingMonsterConfig.monster_name && (
										<span className="text-xs text-dim" style={{ opacity: 0.7 }}>
											· {wanderingMonsterConfig.monster_name}
										</span>
									)}
								</>
					...
					<div className="scenario-toolbar">
						...
						<button
							...
							onClick={() => toggleMode("dungeon")}
							title="Draw a polygon on the map to define a dungeon, then describe it to the AI"
							style={placementMode === "dungeon" ? { borderColor: "#ff6d00", color: "#ff6d00" } : {}}
						>
							+ Dungeon
						</button>
					</div>
				...
				{placementMode === "none" && !selectedTemplate && !chatContext && !dungeonContext && (
					<div className="scenario-hint-bar">
						Select <strong>+ Enemy</strong>, <strong>+ NPC</strong>,{" "}
						<strong>+ Friendly</strong>, <strong>+ Vehicle</strong>,{" "}
						<strong>+ POI</strong>, or <strong>+ Dungeon</strong> then click the map. Click an existing marker to copy it.
					</div>
				)}
					...
					onConfirm={handleEntityConfirm}
						onConfirmBatch={handleEntityConfirmBatch}
					...
					{dungeonContext && (
						<DungeonDesignerPanel
							...
							onConfirmDungeon={handleDungeonConfirm}
							onCancel={() => setDungeonContext(null)}
						/>
					)}
				</div>
				</div>
