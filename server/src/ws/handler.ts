...
// Pending follow requests from NPCs to players
// Cleared when accepted, declined, or expired (30 seconds)
const pendingFollowRequests = new Map<string, FollowRequest>(); // requestId → FollowRequest
 ...
 */
async function triggerAgentTurnIfNeeded(
	sessionId: string,
	campaignId: string,
): Promise<void> {
 ...
 * The NPC's dialogue should already be sent separately.
 */
export function sendFollowRequest(
	...
	// Send the follow request as a special message type
...
async function handleClientMessage(
	...
	switch (msg.type) {
		...
		case "chat_message": {
			...
			let suppressGmResponse = false; // Flag to skip GM response when tactical mode triggered
			...
			if (actorId && chatMsg.message_type === "player_speech") {
				...
				if (combatant && combatant.status !== "dead") {
					...
					if (movement) {
						...
						if (isTactical) {
							// Tactical mode: move directly without contact detection
							const newX = curX + movement.dx;
							const newY = curY + movement.dy;
							const updated = gameState.updateCombatantPosition(
								actorId,
							...
							// Non-tactical mode: check for contact BEFORE moving
							...
							// Move all party members to the final position (may be stopped early)
							const party = gameState.getPartyCombatants(session.campaign_id);
							...
							// If contact was made, mark the enemy as detected and enter tactical mode
							if (movementResult.contactMade && movementResult.detectedEnemy) {
								...
								console.log(
									`[move] Contact made with ${enemy.name} (id=${enemy.id}) at tactical position (${enemy.tactical_x}, ${enemy.tactical_y})`,
								);
								console.log(
									`[move] Enemy status: detected=${enemy.detected}, status=${enemy.status}, type=${enemy.enemy_type}`,
								);
								...
								if (!enemy.detected) {
									...
									console.log(
										`[move] Broadcasting enemy_update for ${enemy.name} with detected=true, tactical_x=${detectedEntity.tactical_x}, tactical_y=${detectedEntity.tactical_y}`,
									);
									broadcastToSession(client.sessionId, {
										type: "enemy_update",
									...
									console.log(
										`[move] Enemy ${enemy.name} detected at ${movementResult.distanceToEnemy} ft during movement`,
									);
								} else {
									console.log(
										`[move] Enemy ${enemy.name} was already detected, skipping detection broadcast`,
									);
								}

								// Send chat message about the detection
								const spotterName =
								...
								// If hostile and not already in tactical mode, enter it now
								if (isHostile && session.current_mode !== "tactical") {
									console.log(
										`[move] Entering tactical mode for hostile ${enemy.name}`,
									);
									...
									console.log(
										`[move] Party has ${party.length} living members`,
									);
									...
									console.log(
										`[move] Broadcasting mode_change to tactical with turn_order=${turn_order.map((id) => party.find((c) => c.id === id)?.name).join(", ")}`,
									);
									...
									const firstActorName =
										party.find((c) => c.id === turn_order[0])?.name ??
										"Unknown";
									...
									// Request GM to narrate the enemy sighting
									const campaign = gameState.getCampaign(session.campaign_id);
									...
									suppressGmResponse = true; // Don't let GM narrate interrupted movement
									console.log(
										`[move] Hostile contact detected — entering tactical mode`,
									);
								}
							} else {
								// No contact made, proceed with wandering monster check
			...
			// AI GM responds to player speech (unless suppressed by tactical mode entry)
			const campaign = gameState.getCampaign(session.campaign_id);
			if (
				!suppressGmResponse &&
		...
		case "tactical_move": {
			...
			console.log(
				`[tactical_move] combatantId=${client.combatantId?.slice(-4)} target=(${target_x},${target_y}) sessionId=${client.sessionId?.slice(-4)}`,
			);

			if (!client.sessionId || !client.combatantId) {
				console.log(
					"[tactical_move] rejected: not registered (combatantId missing even after payload fallback)",
				);
				client.socket.send(
					JSON.stringify({
						type: "error",
			...
			if (!session || session.current_mode !== "tactical") {
				console.log(
					`[tactical_move] rejected: not in tactical mode (mode=${session?.current_mode})`,
				);
				client.socket.send(
					JSON.stringify({
						type: "error",
			...
			const activeId = ts?.turn_order[ts.current_actor_index ?? 0];
			console.log(
				`[tactical_move] activeId=${activeId?.slice(-4)} combatantId=${client.combatantId?.slice(-4)} match=${activeId === client.combatantId}`,
			);
			...
			console.log(
				`[tactical_move] position updated: ${updated?.name} → (${updated?.tactical_x},${updated?.tactical_y}) | broadcasting to session ${client.sessionId?.slice(-4)}`,
			);
			if (updated) {
				broadcastToSession(client.sessionId, {
					type: "combatant_update",
		...
		case "end_turn": {
			console.log(
				`[end_turn] combatantId=${client.combatantId} sessionId=${client.sessionId}`,
			);
			if (!client.sessionId || !client.combatantId) {
				client.socket.send(
					JSON.stringify({
			...
			console.log(
				`[end_turn] activeId=${activeId} myId=${client.combatantId} match=${activeId === client.combatantId}`,
			);
			if (activeId !== client.combatantId) {
				client.socket.send(
					JSON.stringify({
			...
			console.log(
				`[end_turn] advanced to actor index ${nextIndex} (${newTs.turn_order[nextIndex]})`,
			);
		...
		case "follow_response": {
			const { request_id, accepted } = msg.payload as {
				request_id: string;
				accepted: boolean;
			};
			...
			// Find the follow request from pending requests
			...
			// Validate distance (400m = ~1312 ft max)
			...
			// Move player to the NPC's location
			const updated = gameState.updateCombatantPosition(
				client.combatantId,
				followReq.target_x,
				followReq.target_y,
