...
export class GameStateService {
  ...
  createCampaign(req: CreateCampaignRequest): Campaign {
    ...
    db.prepare(`
      INSERT INTO campaigns (id, name, creator_user_id, gm_kind, gm_user_id, gm_agent_config, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, req.name, req.gm_user_id || 'system', req.gm_kind, req.gm_user_id || null,
      req.gm_agent_config ? JSON.stringify(req.gm_agent_config) : null, now, now);
  ...
  getCampaign(id: string): Campaign | null {
    ...
    return {
      ...
      gm_agent_config: row.gm_agent_config ? JSON.parse(row.gm_agent_config) : undefined,
    };
  }
  ...
  getActiveSession(campaignId: string): Session | null {
    const db = getDb();
    const row = db.prepare('SELECT * FROM sessions WHERE campaign_id = ? AND active = 1').get(campaignId) as any;
    if (!row) return null;
    return {
      ...row,
      active: Boolean(row.active),
  ...
  updateSessionMode(sessionId: string, mode: GameMode): void {
    const db = getDb();
    db.prepare('UPDATE sessions SET current_mode = ?, updated_at = ? WHERE id = ?')
      .run(mode, new Date().toISOString(), sessionId);
  }

  updateTurnState(sessionId: string, turnState: TurnState | null): void {
    const db = getDb();
    db.prepare('UPDATE sessions SET turn_state = ?, updated_at = ? WHERE id = ?')
      .run(turnState ? JSON.stringify(turnState) : null, new Date().toISOString(), sessionId);
  }
  ...
  addSpectator(sessionId: string, userId: string): void {
    ...
    if (!specs.includes(userId)) {
      specs.push(userId);
      db.prepare('UPDATE sessions SET spectator_user_ids = ?, updated_at = ? WHERE id = ?')
  ...
  createCombatant(req: CreateCombatantRequest & {
    attributes?: Record<string, number>;
    hp?: number; sdc?: number; isp?: number; ppe?: number;
    apm?: number; combat_bonuses?: Record<string, number>;
    psionic_powers?: string[]; skills?: string[];
    inventory?: InventoryItem[]; equipped?: Record<string, string>;
    ...
    `).run(
      id, req.campaign_id, req.kind, req.controller || null, 'dog_boy', req.name,
      req.personality ? JSON.stringify(req.personality) : null,
      req.attributes?.iq ?? 10, req.attributes?.me ?? 11, req.attributes?.ma ?? 9,
      req.attributes?.ps ?? 12, req.attributes?.pp ?? 12, req.attributes?.pe ?? 13,
      req.attributes?.pb ?? 7, req.attributes?.spd_bipedal ?? 22, req.attributes?.spd_quadruped ?? 40,
      req.hp ?? 13, req.hp ?? 13, req.sdc ?? 40, req.sdc ?? 40,
      req.isp ?? 21, req.isp ?? 21, req.ppe ?? 18, req.ppe ?? 18,
      req.armor_mdc ?? 70, req.armor_mdc ?? 70,
      req.apm ?? 4,
  ...
  updateCombatantVitals(id: string, updates: Partial<{
    hp_current: number; sdc_current: number; isp_current: number; ppe_current: number;
    armor_mdc_current: number; hunger: number; thirst: number; fatigue: number;
    internal_temp: number; pulse_bpm: number; status: CombatantStatus;
  }>): void {
    const db = getDb();
    ...
    db.prepare(`UPDATE combatants SET ${fields}, updated_at = ? WHERE id = ?`)
      .run(...values, new Date().toISOString(), id);
  }

  updateCombatantPosition(id: string, x: number, y: number, facing?: string): Combatant | null {
    const db = getDb();
    db.prepare('UPDATE combatants SET tactical_x = ?, tactical_y = ?, facing = ?, updated_at = ? WHERE id = ?')
      .run(x, y, facing || null, new Date().toISOString(), id);
    return this.getCombatant(id);
  }

  updateCombatantXP(id: string, xpGain: number): { leveled_up: boolean; new_level?: number } {
    const db = getDb();
    ...
    db.prepare('UPDATE combatants SET xp = ?, level = ?, updated_at = ? WHERE id = ?')
      .run(newXp, newLevel, new Date().toISOString(), id);

    return { leveled_up: leveledUp, new_level: leveledUp ? newLevel : undefined };
  }

  updateCombatantInventory(id: string, inventory: InventoryItem[]): void {
    const db = getDb();
    db.prepare('UPDATE combatants SET inventory = ?, updated_at = ? WHERE id = ?')
      .run(JSON.stringify(inventory), new Date().toISOString(), id);
  }

  updateCombatantPowers(id: string, powers: string[]): void {
    const db = getDb();
    db.prepare('UPDATE combatants SET psionic_powers = ?, updated_at = ? WHERE id = ?')
      .run(JSON.stringify(powers), new Date().toISOString(), id);
  }

  updatePackHowl(id: string, remaining: number): void {
    const db = getDb();
    db.prepare('UPDATE combatants SET pack_howl_remaining = ?, updated_at = ? WHERE id = ?')
  ...
  addInjury(combatantId: string, injury: Omit<Injury, 'id'>): Injury {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, combatantId, injury.body_location, injury.severity, injury.injury_type,
      injury.bleeding ? 1 : 0, injury.pain_level, injury.healing_progress);
    return { id, ...injury };
  }
  ...
  createMessage(req: SendMessageRequest): ChatMessage {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, req.campaign_id, req.session_id || null, req.actor_id || null,
      req.message_type, req.content, req.visibility || 'party', now);
    return {
      id, campaign_id: req.campaign_id, session_id: req.session_id,
      actor_id: req.actor_id, message_type: req.message_type,
      content: req.content, visibility: req.visibility || 'party', created_at: now,
    };
  }
  ...
  logEvent(event: Omit<GameEvent, 'id' | 'created_at'>): GameEvent {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, event.campaign_id, event.session_id || null, event.event_type,
      event.actor_id || null, event.target_id || null,
      event.data ? JSON.stringify(event.data) : null,
      event.narrative || null, event.visibility || 'party', now);
  ...
  createEnemy(sessionId: string, enemy: Partial<Enemy> & { name: string; enemy_type: string; hp_max: number }): Enemy {
    const db = getDb();
    const id = uuid();
    db.prepare(`
    ...
    `).run(
      ...
      enemy.apm ?? 2, enemy.initiative_bonus ?? 0,
      enemy.strike_bonus ?? 0, enemy.parry_bonus ?? 0, enemy.dodge_bonus ?? 0,
      enemy.damage ?? '1d6', enemy.damage_type ?? 'sdc',
      enemy.tactical_x ?? null, enemy.tactical_y ?? null,
      JSON.stringify(enemy.abilities ?? []), JSON.stringify(enemy.loot_table ?? [])
    );
  ...
  updateEnemyHp(id: string, hp: number, status?: CombatantStatus): void {
    const db = getDb();
    if (status) {
      db.prepare('UPDATE enemies SET hp_current = ?, status = ? WHERE id = ?').run(hp, status, id);
    } else {
  ...
  setTerrain(sessionId: string, tiles: TacticalTile[]): void {
    ...
    const batch = db.transaction((t: TacticalTile[]) => {
      for (const tile of t) {
        stmt.run(sessionId, tile.x, tile.y, tile.terrain_type, tile.cover || null,
          tile.elevation, tile.revealed ? 1 : 0, tile.metadata ? JSON.stringify(tile.metadata) : null);
      }
    });
    batch(tiles);
  }
  ...
  getTerrain(sessionId: string): TacticalTile[] {
    ...
    return rows.map(r => ({
      x: r.x, y: r.y, terrain_type: r.terrain_type,
      cover: r.cover || null, elevation: r.elevation,
  ...
  recordVitalSample(combatantId: string, pulseBpm: number, internalTemp: number): void {
    const db = getDb();
    db.prepare('INSERT INTO vital_samples (combatant_id, pulse_bpm, internal_temp) VALUES (?, ?, ?)')
  ...
  pruneVitalSamples(olderThanMinutes = 30): void {
    const db = getDb();
    db.prepare("DELETE FROM vital_samples WHERE sampled_at < datetime('now', '-' || ? || ' minutes')")
  ...
  private rowToCombatant(row: any): Combatant {
    return {
      ...
      attributes: {
        iq: row.iq, me: row.me, ma: row.ma, ps: row.ps, pp: row.pp,
        pe: row.pe, pb: row.pb, spd_bipedal: row.spd_bipedal, spd_quadruped: row.spd_quadruped,
      },
      vitals: {
        hp_current: row.hp_current, hp_max: row.hp_max,
        sdc_current: row.sdc_current, sdc_max: row.sdc_max,
        isp_current: row.isp_current, isp_max: row.isp_max,
        ppe_current: row.ppe_current, ppe_max: row.ppe_max,
        armor_mdc_current: row.armor_mdc_current, armor_mdc_max: row.armor_mdc_max,
      },
      combat: {
